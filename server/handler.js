...
handler.sendTopUserStats = function(req,res){
  ...
  fs.readFile('./leaderboard-data/userStats/top-user-stats-'+encodeURIcomponent(language)+'.json',
    function(err,data){
...
handler.sendTopRepoStats = function(req,res){
  var language = req.body.language;
  fs.readFile('./leaderboard-data/repoStats/top-repo-stats-'+encodeURIcomponent(language)+'.json',
    function(err,data){
    if(err) throw new Error(err);   
    res.set({
      'Content-Type': 'text/json'
